...
- **Auth**: Clerk (JWT-based). Frontend uses `@clerk/react` (v6+). Backend validates the JWT in Axum middleware, extracts `clerk_id`, maps to internal `user_id`. Do not use the old `@clerk/clerk-react` package.
